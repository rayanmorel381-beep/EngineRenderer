...
impl ColorGradient {
    ...
    pub fn constant(color: Vec3, alpha: f64) -> Self {
        Self {
            stops: vec![(0.0, color, alpha), (1.0, color, alpha)],
        }
    }
...
impl GpuParticleSystem {
    ...
    pub fn update(&mut self, dt: f64) {
        ...
        for emitter in &mut self.emitters {
            let speed_scale = if emitter.simulation_space == local_space {
                0.9
            } else {
                1.0
            };
            ...
            for _ in 0..count {
                let lifetime = emitter.lifetime_min
                    + (emitter.lifetime_max - emitter.lifetime_min) * lcg_next(&mut seed);
                let speed = (emitter.initial_speed_min
                    + (emitter.initial_speed_max - emitter.initial_speed_min)
                        * lcg_next(&mut seed))
                    * speed_scale;
                let dir =
                    cone_direction(emitter.emission_direction, emitter.spread_angle, &mut seed);
        ...
        for particle in &self.particles {
            ...
            for dy in -radius..=radius {
                for dx in -radius..=radius {
                    ...
                    let sx = px + dx;
                    let sy = py + dy;
                    if sx < 0 || sx >= fb.width as i64 || sy < 0 || sy >= fb.height as i64 {
                        continue;
                    }
