...
use super::math::Vec3;
use super::primitives::{EPSILON, Material, Ray};
use super::scene::{DirectionalLight, Scene};
use crate::core::engine::rendering::{
    lod::manager::LodManager, lod::selection::LodSelection,
...
pub struct TraceContext<'a> {
    ...
    pub sdf: Option<&'a crate::core::engine::rendering::sdf::world_sdf::WorldSdf>,
}
...
pub fn shade_hit(
    ...
    let effective_roughness =
        (hit.material.roughness * 0.68 + micro_roughness * 0.32).clamp(0.02, 0.98);
    ...
    let (fresnel, specular_strength, clearcoat_highlight) = compute_specular(
        normal,
        light_direction,
        view_direction,
        hit.material,
    ...
    let diffuse = base_color
        * (1.0 - hit.material.metallic.clamp(0.0, 1.0))
        * (1.0 - hit.material.transmission * 0.35);
    let specular_term =
        fresnel * specular_strength * lod.reflection_boost + Vec3::splat(clearcoat_highlight);
    let direct_light = ((diffuse * n_dot_l) + specular_term)
        * trace.scene.sun.color
        * trace.scene.sun.intensity
        * visibility;

    let subsurface_light = compute_subsurface(
        normal,
        light_direction,
        base_color,
        hit.material,
    ...
    let ao = if depth == 0 && lod.ao_samples > 0 && hit.material.ambient_occlusion > 0.01 {
        ambient_occlusion(
            ...
            trace.seed,
            trace.bvh,
        ) * hit.material.ambient_occlusion
    } else {
        1.0
    };
    ...
    let volume_light = trace
        .scene
        .volume
        .inscattering(ray, hit.distance.max(0.0), trace.scene.sun);
    let fast_density = trace
        .scene
        .volume
        .local_density_fast(ray.at(hit.distance * 0.5));
    let sigma = fast_density * (1.0 + trace.scene.volume.absorption);
    let transmittance = (-sigma * hit.distance.max(0.0) * 0.18)
        .exp()
        .clamp(0.0, 1.0);

    let mut shaded = (hit.material.emission
        + direct_light
        + subsurface_light
        + area_light
        + diffuse * ambient
    ...
    if hit.material.transmission > 0.01 && depth + 1 < local_limit {
        let refracted_direction = ray
            .direction
            .refract(normal, 1.0 / hit.material.ior.max(1.01))
            .normalize();
        ...
        let transmitted = trace_ray(
            ...
            depth + 1,
            TraceContext {
                seed: trace.seed ^ 0x51ED_270B,
                ..trace
            },
        );
        shaded = shaded.lerp(
            transmitted + ambient * 0.25,
            hit.material.transmission.clamp(0.0, 0.85),
        );
    }

    if depth + 1 < local_limit {
        let reflection_weight =
            (hit.material.reflectivity + hit.material.clearcoat * 0.18).clamp(0.0, 1.2);
        if reflection_weight > 0.01 {
            let reflected = ray.direction.reflect(normal).normalize();
            let glossy_direction = (reflected
                + random_in_unit_sphere(trace.seed ^ 0x9E37_79B9) * effective_roughness)
                .normalize();
            let reflected_ray = Ray::new(hit.point + normal * EPSILON * 3.0, glossy_direction);
            let reflected_light = trace_ray(
                reflected_ray,
                depth + 1,
                TraceContext {
                    seed: trace.seed ^ 0x85EB_CA6B,
                    ..trace
                },
            );
...
pub fn trace_ray(ray: Ray, depth: u32, trace: TraceContext<'_>) -> Vec3 {
    if let Some(hit) = BvhNode::hit_scene(trace.scene, &ray, EPSILON, f64::INFINITY, trace.bvh) {
        return shade_hit(ray, &hit, depth, trace);
    }
    if let Some(sdf) = trace.sdf {
        if let Some((sdf_t, sdf_normal)) = sdf.march(ray.origin, ray.direction, 200.0, 128) {
            let hit_point = ray.at(sdf_t);
            let sun_dir = (-trace.scene.sun.direction).normalize();
            let n_dot_l = sdf_normal.dot(sun_dir).max(0.0);
            let irr_hint = sdf.sample_irradiance_hint(hit_point, 2.0);
            let sky_contrib = sky_color(trace.scene, sdf_normal, trace.lod_manager);
            return (trace.scene.sun.color * (n_dot_l * trace.scene.sun.intensity * 0.7)
                + sky_contrib * (0.3 + irr_hint * 0.4))
                .clamp(0.0, 8.0);
        }
    }
...
fn compute_specular(
    ...
    let specular_strength =
        normal.dot(half_vector).max(0.0).powf(specular_power) * anisotropy_shape;
    ...
    let fresnel = fresnel.lerp(
        fresnel * (Vec3::splat(0.72) + iridescent_tint * 0.9),
...
pub fn area_light_contribution(input: AreaLightSample<'_>) -> Vec3 {
    ...
    for (li, light) in input.scene.area_lights.iter().enumerate() {
        ...
        for si in 0..sample_count {
            let ss = input.seed
                ^ ((li as u32 + 1).wrapping_mul(0x7FEB_352D))
                ^ ((si as u32 + 1).wrapping_mul(0x846C_A68B));
            ...
            let th = if input.normal.x.abs() > input.normal.z.abs() {
                Vec3::new(-input.normal.y, input.normal.x, 0.0)
            } else {
                Vec3::new(0.0, -input.normal.z, input.normal.y)
            };
            let t = th.normalize();
            let bt = input.normal.cross(t).normalize();
            let aniso = input.material.anisotropy.clamp(0.0, 1.0);
            let ab = (hv.dot(t).abs() * (1.0 + aniso * 1.5)
                + hv.dot(bt).abs() * (1.0 - aniso * 0.5))
                .clamp(0.4, 2.0);
            let sp = 24.0 + (1.0 - input.material.roughness.clamp(0.02, 0.98)) * 160.0;
            let spec = input.normal.dot(hv).max(0.0).powf(sp)
                * (0.22 + input.material.reflectivity * 0.78 + input.material.clearcoat * 0.35)
                * ab;
            let fresnel =
                Vec3::splat(0.04).lerp(input.base_color, input.material.metallic.clamp(0.0, 1.0));
            let rf = (1.0 - input.normal.dot(input.view_direction).max(0.0)).powf(3.0);
            let irid = Vec3::new(
                (rf * 7.0).sin() * 0.5 + 0.5,
                (rf * 7.0 + 2.1).sin() * 0.5 + 0.5,
                (rf * 7.0 + 4.2).sin() * 0.5 + 0.5,
            );
            let fresnel = fresnel.lerp(
                fresnel * (Vec3::splat(0.7) + irid * 0.95),
                input.material.iridescence,
            );
            ...
            light_total +=
                (diff + sss + fresnel * spec * input.lod.reflection_boost) * light.color * atten;
        }
...
pub fn indirect_diffuse(input: IndirectDiffuseInput<'_>) -> Vec3 {
    ...
    for i in 0..total {
        ...
        let id =
            (input.normal * 0.55 + sun_dir * 0.30 + random_in_unit_sphere(ds ^ 0xA24B_AED4) * 0.15)
                .normalize();
        ...
        let bounced = if let Some(hit) =
            BvhNode::hit_scene(input.scene, &br, EPSILON, 16.0, input.bvh)
        {
            let ba = hit.material.albedo;
            let bndl = hit.normal.dot(sun_dir).max(0.0);
            let sr = Ray::new(hit.point + hit.normal * EPSILON * 4.0, sun_dir);
            let bv = if BvhNode::any_hit(input.scene, &sr, 200.0, input.bvh) {
                0.3
            } else {
                1.0
            };
            (ba * bndl * bv * 0.60
                + hit.material.emission * 0.24
                + sky_color(input.scene, dir, input.lod_manager) * 0.16)
                * cosine
        } else if let Some(sdf) = input.sdf {
            if let Some((_sdf_t, sdf_normal)) = sdf.march(br.origin, br.direction, 32.0, 64) {
                let sdl = sdf_normal.dot(sun_dir).max(0.0);
                sky_color(input.scene, sdf_normal, input.lod_manager) * (0.4 + sdl * 0.6) * cosine
            } else {
...
pub fn caustic_estimate(scene: &Scene, point: Vec3, normal: Vec3, view_direction: Vec3) -> Vec3 {
    ...
    for obj in &scene.objects {
        let focus = obj.material.transmission * 1.35
            + obj.material.clearcoat * 0.45
            + obj.material.reflectivity * 0.20;
        if focus <= 0.08 {
            continue;
        }
...
pub fn sky_color(scene: &Scene, direction: Vec3, lod_manager: &LodManager) -> Vec3 {
    ...
    let horizon_haze = scene.sky_bottom.lerp(scene.sky_top, 0.35)
        * (1.0 - direction.y.abs()).powf(3.0)
        * 0.35
        * horizon_factor;
    let sun_alignment = direction.dot(sun_direction).max(0.0);
    let sun_disc = scene.sun.color
        * scene.sun.intensity
        * sun_alignment.powf(2200.0 * (1.0 - scene.sun.angular_radius).max(0.2));
...
fn perturb_normal(
    ...
    let texture_normal = tex.sample_normal_uv(
        point * (0.35 + lod.texture_frequency * 0.08),
        uv,
        material.uv_scale,
    );
    (normal
        + wave
        + texture_normal * material.normal_map_strength * (0.8 + material.clearcoat * 0.3))
        .normalize()
}

fn star_field(direction: Vec3) -> f64 {
    let hash = direction.x.to_bits()
        ^ direction.y.to_bits().rotate_left(11)
        ^ direction.z.to_bits().rotate_left(19)
        ^ 0xA511_E9B3;
