...
impl MorphTarget {
    pub fn new(name: &str, base: &MeshAsset, target_positions: &[Vec3]) -> Self {
        let delta_positions = base.vertices.iter().zip(target_positions.iter())
            .map(|(v, &tp)| tp - v.position)
            .collect();
        let delta_normals = vec![Vec3::ZERO; base.vertices.len()];
        Self { name: name.to_string(), delta_positions, delta_normals }
    }

    pub fn with_normals(mut self, base: &MeshAsset, target_normals: &[Vec3]) -> Self {
        self.delta_normals = base.vertices.iter().zip(target_normals.iter())
...
impl MorphState {
    pub fn new(targets: Vec<MorphTarget>) -> Self {
        let n = targets.len();
        Self { targets, weights: vec![0.0; n] }
    }
    ...
    pub fn apply(&self, base: &MeshAsset) -> Vec<Vertex> {
        ...
        for (target, &weight) in self.targets.iter().zip(self.weights.iter()) {
            if weight < f64::EPSILON { continue; }
        ...
        for v in &mut vertices {
            ...
            if len > f64::EPSILON { v.normal = v.normal * (1.0 / len); }
        }
        vertices
    }
