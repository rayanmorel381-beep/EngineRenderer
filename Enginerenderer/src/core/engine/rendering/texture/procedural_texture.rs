...
impl ProceduralTexture {
    ...
    pub fn sample_uv(&self, point: Vec3, uv: Option<(f64, f64)>, uv_scale: f64) -> Vec3 {
        ...
        let secondary = ((point.y * 0.55 - point.z * 0.25) * self.scale * 1.37).cos() * 0.5 + 0.5;
        ...
        match self.kind {
            ...
            TextureKind::RockyMineral => self.base_color.lerp(self.accent_color, blend.powf(0.85)),
    ...
    pub fn sample_normal_uv(&self, point: Vec3, uv: Option<(f64, f64)>, uv_scale: f64) -> Vec3 {
        ...
        let wave_x =
            ((point.x + point.y * 0.2 + u * 2.0) * self.scale * 1.15).sin() * self.detail_boost;
        let wave_y = ((point.y - point.z * 0.3 + v * 1.7) * self.scale * 0.92).cos()
            * self.detail_boost
            * 0.6;
        let wave_z =
            ((point.z + point.x * 0.4 + (u - v) * 1.4) * uv_phase * 1.31).sin() * self.detail_boost;
    ...
    pub fn sample_roughness_uv(&self, point: Vec3, uv: Option<(f64, f64)>, uv_scale: f64) -> f64 {
        let (u, v) = uv.unwrap_or((point.x.abs().fract(), point.z.abs().fract()));
        let micro =
            ((point.x - point.z + (u - v) * uv_scale * 2.0) * self.scale * 0.75).sin() * 0.5 + 0.5;
