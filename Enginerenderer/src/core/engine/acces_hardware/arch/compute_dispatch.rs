...
pub(crate) fn default_display_config() -> DisplayConfig {
    let cpu = default_cpu_config();
    let vsync_slots = match cpu.vendor {
        Vendor::Amd => 4,
        Vendor::Intel | Vendor::Apple | Vendor::Unknown => 2,
    };
    DisplayConfig {
        vendor: cpu.vendor,
        page_size: 4096,
...
pub(crate) fn build_display_schedule(work_items: usize) -> Schedule {
    let chunk_size = if work_items == 0 {
        ...
        work_items.div_ceil(16).saturating_mul(16).max(16)
    };
    let chunks = if work_items == 0 {
        1
    } else {
        work_items.div_ceil(chunk_size).max(1)
    };
    Schedule {
        chunks,
        chunk_size,
...
pub(crate) fn clamp_display_workers(requested: usize) -> usize {
    let max = match default_cpu_config().vendor {
        Vendor::Amd => 4,
        Vendor::Intel | Vendor::Apple | Vendor::Unknown => 2,
    };
