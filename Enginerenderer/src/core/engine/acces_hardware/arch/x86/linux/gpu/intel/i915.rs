//! `i915` kernel driver bindings (Gen3 through Gen12 / Xe-LP).
//! Uses the `DRM_IOCTL_I915_GEM_*` and `EXECBUFFER2` ioctl family.
...
pub(crate) fn probe_i915_telemetry(card: &str) -> (u32, u32, u32, i32) {
    ...
    let temp = std::fs::read_dir(format!("{}/hwmon", base))
        .ok()
